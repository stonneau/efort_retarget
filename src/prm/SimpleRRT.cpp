...
namespace planner
{
	...
	struct PImpl
	{
            ...
            : planner_(objects, collisionObjects, *from)
        {
            ...
            bounds.push_back(-1.6);bounds.push_back(-1.22524);

            //bounds.push_back(-3.1);bounds.push_back(3.);
            bounds.push_back(-0.1);bounds.push_back(1);
            ...
            bounds.push_back(std::min(pt.x(),pf.x()-10));bounds.push_back(std::max(pt.x(),pf.x())+20);
            bounds.push_back(std::min(pt.y(),pf.y()));bounds.push_back(std::max(pt.y(),pf.y()));
